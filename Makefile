.PHONY: clean-so clean-test clean-pyc clean-build clean
.PHONY: check check-manifest check-setup check-history lint
...
install-reqs:
	...
	$(foreach req,$(REQUIREMENTS_TXT),pip3 install -r $(req);)
